...
namespace DAL
{
    public class Activity
    {
        ...
        public IEnumerable<Model.Activity> GetActivities(Model.Page page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
        ...
        public IEnumerable<Model.Activity> GetNew()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select count(1) from activity where activityVerify='审核通过' and endTime>=now() order by endTime desc limit 2";
        ...
        public Model.Activity GetRecommend()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select count(1) from activity where activityVerify='审核通过' and recommend='是' and endTime>=now() order by recommendTime desc limit 1";
        ...
        public Model.Activity GetEnd()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select count(1) from activity where activityVerify='审核通过' and endTime<now() order by endTime desc limit 1";
                return cn.QueryFirstOrDefault<Model.Activity>(sql);
            }
        }
